...
const RotatingTrophy = ({ gradient }: { gradient: string }) => (
  ...
  >
    ...
    <span className="relative z-10 text-[24px] md:text-[28px] drop-shadow-md">🏆</span>
  </motion.span>
);
...
const SponsorCard = ({ src, alt, index, isPlaceholder = false }: { src?: string; alt?: string; index: number; isPlaceholder?: boolean }) => (
  ...
  >
    {!isPlaceholder && src ? (
      ...
      <span className="text-gray-300 text-[10px] font-bold uppercase tracking-[0.2em]">Coming Soon</span>
    )}
  </motion.div>
);
...
export default function Sponsors() {
  ...
  const tierGradients: Record<string, string> = {
    "Platinum Sponsors": "linear-gradient(135deg, #CFD1D3 0%, #FFFFFF 50%, #A2A7AB 100%)",
    "Diamond Sponsors": "linear-gradient(135deg, #B9F2FF 0%, #FFFFFF 50%, #61D2F5 100%)",
    "Gold Sponsors": "linear-gradient(135deg, #FFD700 0%, #FFF9C4 50%, #D4AF37 100%)",
    "Silver Sponsors": "linear-gradient(135deg, #C0C0C0 0%, #F5F5F5 50%, #909090 100%)",
  };
    ...
    <section className="w-full bg-[#F3F4F6] py-20 md:py-32 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 space-y-4">
        ...
        {sponsorTiers.map((tier, tierIdx) => {
          ...
          const displayData = tier.data.length > 0 ? tier.data : Array(6).fill(null); 

          return (
            <div key={tierIdx} className="border-b border-gray-200/60 last:border-0 pb-4">
              <button 
